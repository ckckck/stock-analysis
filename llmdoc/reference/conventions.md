...
- 普通图表 K 线链路与 AI 筛选同步链路分开维护：前者默认仍先走 `quotes.sina.cn`，但遇到 `456`、`blocked` 或 `拒绝访问` 时会自动回退到 Eastmoney；后者仍是 `Baostock -> 新 Sina 日线接口`。普通 K 线 fallback 诊断日志必须在默认 `INFO` 下保留 `requestId`、`sinaUrl`、`responsePreview` 和 `fallbackUrl`，这样才能复盘“curl 直调成功、程序调用失败”的差异，见 `jcp/internal/services/market_service.go:517`、`jcp/internal/services/market_service.go:675`、`jcp/internal/services/market_service_test.go:204`。
