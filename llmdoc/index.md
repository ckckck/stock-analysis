...
- `architecture/system-map.md`：桌面壳、前端、服务编排、AI 筛选链、会议系统和 OpenClaw 的执行链，已补 AI 筛选同步的 `Baostock -> Sina` provider 链，以及 `screening:sync:progress` / `screening:query:progress` 事件流、连续思考摘要阶段、空结果状态与历史 SQL 重跑链
- `guides/common-workflows.md`：开发运行、AI 筛选同步/查询、发起分析、调整策略、使用 OpenClaw 的常见流程，已补顶部同步入口、同步状态判定、统一同步弹框、前端 `vitest` 验证入口，以及历史 SQL 重跑与断点同步的前端交互规则
