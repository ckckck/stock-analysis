...
- `guides/common-workflows.md`：开发运行、AI 筛选同步/查询、发起分析、调整策略、使用 OpenClaw 的常见流程，已补顶部同步入口、同步状态判定、统一同步弹框、前端 `vitest` / 后端 `go test ./...` 验证入口，以及历史 SQL 重跑、删除、范围重选与断点同步的前端交互规则
