...
- AI 筛选同步与普通 K 线获取已分层：同步链路优先追求日线稳定性，因此单独走 `Baostock -> 新 Sina 日线接口`；普通图表与其它 K 线入口仍走 `GetKLineData()`，但当 Sina 返回 `456` / `blocked` / `拒绝访问` 时，会在同一条请求链路里自动回退到 Eastmoney K 线接口。普通 K 线链路现在还增加了三层最小兜底：Eastmoney 在连接类 `EOF`/`connection reset`/`broken pipe` 上会重试，`1d` 比其它周期多一次退避尝试；非分时 K 线会把最后一次成功结果在内存里保留 24 小时，供后续失败时走 `kline.fetch.stale_cache_used` 继续返回旧数据；若同一 symbol/period 刚失败过且仍有 stale 数据，20 秒冷却窗口内会直接走 `kline.fetch.cooldown_stale_cache_used`，避免连续请求反复撞上游。前端只会在重试耗尽且该股票从未成功拿到过任何 K 线时，才弹“请求速度太快触发平台风控限制”的 toast，见 `jcp/internal/services/market_service.go`、`jcp/internal/services/market_service_test.go`、`jcp/frontend/src/App.tsx`、`jcp/frontend/src/utils/klineFallback.ts`。
