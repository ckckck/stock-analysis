...
const (
	...
	ModeratorTimeout     = 2 * time.Minute  // 小散牛分析/总结的最大时长
...
var (
	...
	ErrModeratorTimeout = errors.New("小散牛响应超时")
...
type Service struct {
	...
	moderatorAIConfig *models.AIConfig         // 意图分析(小散牛)使用的 LLM 配置
...
// SetModeratorAIConfig 设置意图分析(小散牛)使用的 LLM 配置
...
// 会议模式常量
const (
	MeetingModeSmart  = "smart"  // 串行智能模式（小散牛编排）
...
func (s *Service) RunSmartMeetingSync(ctx context.Context, aiConfig *models.AIConfig, req ChatRequest) (string, error) {
	...
	if err != nil {
		if errors.Is(err, context.DeadlineExceeded) {
			return "", fmt.Errorf("%w: 小散牛分析超时", ErrModeratorTimeout)
		}
	...
	if len(selectedAgents) == 0 {
		return "", fmt.Errorf("小散牛未选中任何有效专家")
	}
...
func (s *Service) RunSmartMeetingWithCallback(ctx context.Context, aiConfig *models.AIConfig, req ChatRequest, respCallback ResponseCallback, progressCallback ProgressCallback) ([]ChatResponse, error) {
	...
	// 第0轮：小散牛分析意图并选择专家（带超时）
	emitProgress(progressCallback, ProgressEvent{
		Type: "agent_start", AgentID: "moderator", AgentName: "小散牛", Detail: "分析问题意图",
	})
	...
	if err != nil {
		emitProgress(progressCallback, ProgressEvent{
			Type: "agent_done", AgentID: "moderator", AgentName: "小散牛",
		})
		if errors.Is(err, context.DeadlineExceeded) {
			return nil, fmt.Errorf("%w: 小散牛分析超时", ErrModeratorTimeout)
		}
	...
	// 最终轮：小散牛总结（带超时）
	emitProgress(progressCallback, ProgressEvent{
		Type: "agent_start", AgentID: "moderator", AgentName: "小散牛", Detail: "总结讨论",
	})
...
// filterAgentsOrdered 按指定顺序筛选专家（保持小散牛选择的顺序）
...
// runMeetingSummary 执行小散牛总结（ContinueMeeting 专用）
func (s *Service) runMeetingSummary(
	...
	if summary != "" {
		summaryResp := ChatResponse{
			AgentID: "moderator", AgentName: "小散牛",
